    ...
    public function parse()
    {
        ...
        if (null !== $variable) {
            if (count($values) > 1) {
                ...
                $assertions[] = $this->newAssertion($this->context->getCodeContext(), $variable);
            }
        } else {
            if (count($values) > 1) {
                for ($num = count($values), $i = 1; $i <= $num; $i++) {
                    $assertions[] = $this->newAssertion($this->context->getCodeContext(), $values[$i - 1], $num > 1 ? $i : 0);
                }
            } else if (count($values)) {
                if ($iterations = $this->getIterations()) {
                    foreach ($iterations as $iteration) {
                        $assertions[] = $this->newAssertion($this->context->getCodeContext(), $values[0], $iteration);
                    }
                } else {
                    $assertions[] = $this->newAssertion($this->context->getCodeContext(), $values[0]);
                }
            }
        }
